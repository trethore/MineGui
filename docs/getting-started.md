...
- Views persist layout and style snapshots by default. Call `setPersistent(false)` (or the fluent `persistent(false)`) for transient tooling that should leave no saves behind.
